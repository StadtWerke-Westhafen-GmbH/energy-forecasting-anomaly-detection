    ...
    <div className={["sww-tabs", variant === "pills" && "sww-tabs--pills", className].filter(Boolean).join(" ")} role="tablist" {...rest}>
      {items.map((it) => (
        <button key={it.id} className="sww-tabs__t" role="tab" type="button"
          ...
          onKeyDown={(e) => {
            const enabled=items.filter(item=>!item.disabled);const index=enabled.findIndex(item=>item.id===it.id);
            let next;
            if(e.key==='ArrowRight')next=enabled[(index+1)%enabled.length];
            if(e.key==='ArrowLeft')next=enabled[(index+enabled.length-1)%enabled.length];
            if(e.key==='Home')next=enabled[0];if(e.key==='End')next=enabled[enabled.length-1];
            if(next){e.preventDefault();onChange?.(next.id);const buttons=[...e.currentTarget.parentElement.querySelectorAll('[role="tab"]:not(:disabled)')];buttons[enabled.indexOf(next)]?.focus();}
          }}
