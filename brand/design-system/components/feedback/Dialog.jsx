...
export function Dialog({ open = false, title, subtitle, size = "md", footer, onClose, children, className = "", ...rest }) {
  ...
  React.useEffect(() => {
    ...
    return () => { document.removeEventListener("keydown", h); previous?.focus(); };
  }, [open, onClose]);
  if (!open) return null;
  return (
    <div className="sww-dlg__scrim" onClick={onClose ? (e) => { if (e.target === e.currentTarget) onClose(); } : undefined}>
      <div ref={dialogRef} tabIndex={-1} className={["sww-dlg", `sww-dlg--${size}`, className].filter(Boolean).join(" ")} role="dialog" aria-modal="true" aria-label={typeof title === "string" ? title : undefined} {...rest}>
