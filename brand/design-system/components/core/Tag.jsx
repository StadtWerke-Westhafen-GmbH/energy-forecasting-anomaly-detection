...
.sww-tag__action{display:inline-flex;align-items:center;gap:6px;border:0;padding:0;background:transparent;color:inherit;font:inherit;cursor:pointer}
`;
...
export function Tag({ children, dotColor, icon, selected = false, onRemove, onClick, className = "", ...rest }) {
  ...
  const Node = clickable && !onRemove ? "button" : "span";
  const content = <>{dotColor ? <span className="sww-tag__dot" style={{ background: dotColor }} /> : null}{icon ? <Icon name={icon} size={14} /> : null}{children}</>;
  return (
    <Node className={cls} onClick={Node === "button" ? onClick : undefined} type={Node === "button" ? "button" : undefined} {...rest}>
      {clickable && onRemove ? <button type="button" className="sww-tag__action" onClick={onClick}>{content}</button> : content}
      {onRemove ? (
