  ...
  return (
    <>
      <PageHeader eyebrow="Frühwarnung" title={"Anomalien " + monat}
        ...
        actions={<><Button variant="secondary" icon="download">Export CSV</Button><Button variant="primary" icon="file-chart-column">Bericht erzeugen</Button></>} />

      <Alert status="warn" title="Einheiten gemischt" style={{ marginBottom: "var(--space-4)" }}
        actions={<Button size="sm" variant="secondary" icon="database">Zur Datenqualität</Button>}>
      ...
      <Card flush>
        <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", padding: "var(--space-3) var(--gutter-card)", borderBottom: "1px solid var(--border-subtle)", flexWrap: "wrap" }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
            <Input size="sm" icon="search" placeholder="Zähler oder Kunde" value={q} onChange={(e) => setQ(e.target.value)} style={{ width: 210 }} />
            <Select size="sm" value={typ} onChange={(e) => setTyp(e.target.value)} style={{ width: 170 }}
            ...
            <IconButton icon="filter" label="Weitere Filter" bordered />
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", padding: "var(--space-3) var(--gutter-card)", borderTop: "1px solid var(--border-subtle)", font: "var(--text-caption)", color: "var(--text-muted)" }}>
          ...
          <span style={{ marginLeft: "auto", display: "flex", gap: "var(--space-2)" }}>
            <IconButton size="sm" icon="chevron-left" label="Vorherige Seite" bordered />
            <span style={{ font: "var(--text-data)", alignSelf: "center" }}>1 / 8</span>
            <IconButton size="sm" icon="chevron-right" label="Nächste Seite" bordered />
          </span>
        </div>
      </Card>
      ...
      <Dialog open={Boolean(ticket)} onClose={() => setTicket(null)} title="Anomalie-Ticket anlegen"
        ...
        footer={<><Button variant="secondary" onClick={() => setTicket(null)}>Abbrechen</Button><Button variant="primary" icon="check" onClick={() => setTicket(null)}>Ticket anlegen</Button></>}>
        {ticket ? (
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
            <p style={{ margin: 0 }}>
              Abweichung {D.fmtPct(ticket.abweichung_pct)} gegenüber der Prognose ({D.fmt(ticket.prognose_kwh)} kWh).
              Das Ticket geht an Netzmanagement (Anke Bürger).
            </p>
            ...
            <Checkbox label="Netzmanagement per E-Mail informieren" defaultChecked />
          </div>
        ) : null}
      </Dialog>
    </>
