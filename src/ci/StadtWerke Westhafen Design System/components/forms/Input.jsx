  ...
  return (
    <div className="sww-field" style={style}>
      ...
      <div className={cls}>
        ...
        <input className="sww-input__el" id={uid} disabled={disabled} aria-invalid={error ? true : undefined} {...rest} />
        {suffix ? <span className="sww-input__affix">{suffix}</span> : null}
      </div>
      {error ? <span className="sww-field__err"><Icon name="triangle-alert" size={12} />{error}</span>
        : hint ? <span className="sww-field__hint">{hint}</span> : null}
    </div>
  );
}
