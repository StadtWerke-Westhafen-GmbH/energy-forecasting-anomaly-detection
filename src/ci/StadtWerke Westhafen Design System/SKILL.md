...
- `styles.css` → `tokens/*.css` — the complete token set; link `styles.css` and use the custom properties
...
- `assets/plotly/` — `sww_eda.py`: 15 ready EDA figures for notebooks (+ `sww_plotly.js` twin for the browser), Plotly/Matplotlib theme
...
2. Navy = structure, teal = interaction, cyan = **model output only**, green/amber/red = status only.
...
6. For any chart, use `sww_eda.py` / `sww_plotly.js` builders before writing raw Plotly — they encode rules 2–3.
