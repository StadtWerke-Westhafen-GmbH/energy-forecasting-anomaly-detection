...
Use the installed package: from energy_analytics.visualization import eda.

    from energy_analytics.visualization import eda
    eda.setup()                                   # registers + activates template "sww"
    fig = eda.timeseries_forecast(df["monat"], df["ist"], df["prognose"], lo, hi)
    fig.show()
...
SEQ = T.SEQUENTIAL
DIV = T.DIVERGING
GREY_100, GREY_300, GREY_500, GREY_600 = T.GREY_100, T.GREY_300, T.GREY_500, T.GREY_600
...
def timeseries_forecast(
    x, ist, prognose=None, lo=None, hi=None, anomalien=None, y_title="Verbrauch (kWh)", title=None
):
    ...
    fig.update_layout(
        title=title, yaxis_title=y_title, xaxis_title=None, xaxis=dict(nticks=6, tickangle=0)
    )
    return fig.update_layout(template=T.template())
    ...
    for k, v in series.items():
        fig.add_trace(
            go.Scatter(
                ...
                name=k,
                mode="lines+markers",
                line=dict(color=_kt_color(k), width=2),
                marker=dict(size=5),
            )
        )
...
def histogram(groups: dict, x_title="Verbrauch (kWh)", nbins=40, log_x=False, title=None):
    ...
    for k, v in groups.items():
        ...
        fig.add_trace(
            go.Histogram(x=x, name=k, nbinsx=nbins, opacity=0.62, marker_color=_kt_color(k))
        )
    xaxis = (
        dict(
            title=x_title,
        ...
        if log_x
        else dict(title=x_title, tickformat=",d")
    )
    fig.update_layout(
        title=title,
        barmode="overlay",
    ...
    for k, v in groups.items():
        fig.add_trace(
            go.Box(
                y=v,
                name=k,
                marker=dict(color=_kt_color(k), size=3),
                line=dict(width=1.5),
                boxpoints="outliers",
...
# 05 ---------------------------------------------------------------------------
def scatter_trend(
    groups: dict, x_title="Mitteltemperatur (°C)", y_title="Verbrauch (kWh)", title=None
):
...
def heatmap(x, y, z, z_title="kWh", title=None):
    ...
    fig = go.Figure(
        go.Heatmap(
            ...
            y=y,
            z=z,
            colorscale=_scale(SEQ),
            xgap=2,
            ygap=2,
            colorbar=dict(
                title=dict(text=z_title, font=dict(size=11, color=GREY_500)),
                thickness=10,
                len=0.8,
                outlinewidth=0,
                tickformat=",d",
    ...
    fig.update_layout(
        ...
        yaxis=dict(showgrid=False, ticks="", tickformat="", automargin=True),
...
def correlation(corr_df, title=None, show_text=True):
    ...
    fig = go.Figure(
        go.Heatmap(
            ...
            y=labels,
            z=z,
            colorscale=_scale(DIV),
            zmin=-1,
            zmax=1,
            zmid=0,
            text=text,
            texttemplate="%{text}" if show_text else None,
            textfont=dict(family=T.FONT, size=10),
            xgap=2,
            ygap=2,
            colorbar=dict(
                thickness=10,
    ...
    fig.update_layout(
        ...
        hovermode="closest",
        xaxis=dict(
            showline=False, ticks="", tickangle=-35, automargin=True, tickfont=dict(size=10)
        ),
        yaxis=dict(
            showgrid=False,
            ticks="",
...
def residual_hist(residuen, schwelle, x_title="Residuum (kWh)", nbins=40, title=None):
    fig = go.Figure(
        go.Histogram(x=residuen, nbinsx=nbins, marker_color=ROLE["residuum"], name="Residuen")
    )
    for s in (-1, 1):
    ...
    fig.add_annotation(
        ...
        y=1,
        yref="paper",
        text=f"Schwellwert ±{_fmt_de(schwelle)}",
        showarrow=False,
        xanchor="left",
        yanchor="top",
        font=dict(size=11, color=ROLE["schwellwert"]),
    )
    fig.update_layout(
        title=title,
        showlegend=False,
...
def residual_bars(x, residuen, schwelle, y_title="Residuum (kWh)", title=None):
    ...
    fig.update_layout(
        title=title,
        showlegend=False,
        xaxis=dict(nticks=6, tickangle=0),
...
def feature_importance(features, values, x_title="Anteil", title=None):
    ...
    fig = go.Figure(
        go.Bar(
            ...
            x=v,
            orientation="h",
            marker_color=ROLE["ist"],
            text=[_fmt_de(x, 3) for x in v],
    ...
    fig.update_layout(
        ...
        hovermode="closest",
        xaxis=dict(
            title=x_title, showgrid=True, gridcolor=GREY_100, showline=False, tickformat=",.2f"
        ),
        yaxis=dict(showgrid=False, tickformat="", automargin=True, tickfont=LABEL_FONT),
...
def missing_values(columns, pct, title="Anteil fehlerhaft/fehlend (%)"):
    ...
    colors = [
        ROLE["anomalie"] if v >= 5 else ROLE["schwellwert"] if v >= 1 else GREY_300 for v in p
    ]
    fig = go.Figure(
        go.Bar(
            y=cols,
...
def parity(ist, prognose, anomalie=None, x_title="Prognose (kWh)", y_title="Ist (kWh)", title=None):
    ...
    colors = (
        ROLE["ist"]
        if anomalie is None
        else np.where(np.asarray(anomalie, bool), ROLE["anomalie"], ROLE["ist"])
    )
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(
...
def anomaly_stack(x, series: dict, title=None):
    ...
    fig.update_layout(
        title=title, barmode="stack", yaxis_title="Anomalien", xaxis=dict(nticks=6, tickangle=0)
    )
    return fig.update_layout(template=T.template())
...
def top_n(labels, values, x_title="Verbrauch (kWh)", title=None):
    ...
    fig.update_layout(
        ...
        hovermode="closest",
        xaxis=dict(
            title=x_title, showgrid=True, gridcolor=GREY_100, showline=False, tickformat=",d"
        ),
        yaxis=dict(showgrid=False, tickformat="", automargin=True, tickfont=LABEL_FONT),
        margin=dict(l=20, r=70),
    )
    return fig.update_layout(template=T.template())
    ...
    for k, v in series.items():
        fig.add_trace(
            go.Scatter(
                ...
                stackgroup="one",
                groupnorm="percent",
                line=dict(width=0.5, color=_kt_color(k)),
                fillcolor=_kt_color(k),
            )
        )
    fig.update_layout(
        title=title,
        yaxis=dict(title=y_title, ticksuffix=" %", tickformat=",.0f"),
        xaxis=dict(nticks=6, tickangle=0),
    )
    return fig.update_layout(template=T.template())
...
def by_kundentyp(df, value_col, type_col="kundentyp"):
    ...
    return {
        k: df.loc[df[type_col] == k, value_col].dropna().values
        for k in ("Gewerbe", "Industrie", "Kommunal")
        if (df[type_col] == k).any()
    }
...
def save_for_slide(fig, path, width=1120, height=560, scale=2):
    ...
    export.update_layout(
        font_size=20, legend_font_size=20, xaxis_tickfont_size=18, yaxis_tickfont_size=18
    )
    export.write_image(path, width=width, height=height, scale=scale)
    return path
