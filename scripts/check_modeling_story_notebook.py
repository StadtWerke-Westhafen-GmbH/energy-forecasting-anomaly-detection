"""Execute and validate the presentation-first modeling story notebook.

Execution happens in memory by default. Pass ``--update`` to persist only the
successfully verified outputs in ``notebooks/11_modeling_pruefungsstory.ipynb``.
"""
...
TARGET = ROOT / "notebooks" / "11_modeling_pruefungsstory.ipynb"
...
def main() -> None:
    ...
    kernel_env = dict(
        os.environ,
        IPYTHONDIR=str(ROOT / ".build" / "ipython-modeling-story"),
    )
